...
var baseFuncs = map[string]GoFunction{
	"print":  basePrint,  // 常规打印
	"select": baseSelect, // 返回从index开始的全部可选参数，如果是“#”返回长度
	// 错误处理
	"assert": baseAssert, // 契约错误
	"pcall":  basePCall,  // 保护调用
	"xpcall": baseXPCall, // 带消息的保护调用
	"error":  baseError,  // 错误
	// 迭代器相关
	"ipairs": baseIPairs, // 迭代器递增（键值从1开始，递增1直到索取val为nil）
	"pairs":  basePairs,  // 迭代器
	"next":   baseNext,   // 获取表的下一个键值对
	// 加载函数
	"load":     baseLoad,     // 基础加载块
	"loadfile": baseLoadFile, // 加载文件
	"dofile":   baseDoFile,   // 加载文件并运行
	// 元编程相关
	"getmetatable": baseGetMetatable, // 获取元表
	"setmetatable": baseSetMetatable, // 设置元表
	"rawequal":     baseRawEqual,     // 检查两处索引的值是否相同 以下带Raw的都是只获取不压入栈中
	"rawlen":       baseRawLen,       // 获取指定索引的值长度（只支持表和字符串）
	"rawget":       baseRawGet,       // 从表中获取指定值
	"rawset":       baseRawSet,       // 设置表中某个键的值
	// 类型相关
	"type":     baseType,     // 获取栈顶元素类型
	"tostring": baseToString, // 转化为字符串
	"tonumber": baseToNumber, // 转化为数字
	/* placeholders */
	"_G":       nil,
	"_VERSION": nil,
}
