...
type opcode struct {
	...
	setAFlag byte                              // instruction set register A 寄存器A
	argBMode byte                              // B arg mode	参数B操作数类型
	argCMode byte                              // C arg mode 参数C操作数类型
	opMode   byte                              // op mode 操作模式
	name     string                            // 指令名
	action   func(i Instruction, vm api.LuaVM) // 调用函数
}
