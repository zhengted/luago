...
type funcInfo struct {
	...
	breaks    [][]int                // break表，记录跳转指令的地址记录
	insts     []uint32               // 指令表
	parent    *funcInfo
	upvalues  map[string]upvalInfo // uv表
	subFuncs  []*funcInfo
	numParams int
	isVararg  bool
}
...
// indexOfConstant：根据常量返回键，如果找不到就当插入了
...
// allocReg:分配寄存器 返回的是寄存器的索引 所以需要-1
...
// allocRegs:连续分配n个寄存器，返回第一个寄存器的索引
...
// freeRegs:连续分配n个寄存器，返回第一个寄存器的索引
...
// slotOfLocVar:检查局部变量名是否已经和寄存器绑定
...
func (self *funcInfo) exitScope() {
	...
	for _, pc := range pendingBreakJmps {
		sBx := self.pc() - pc
		i := (sBx+MAXARG_sBx)<<14 | a<<16 | OP_JMP
...
func (self *funcInfo) removeLocVar(locVar *locVarInfo) {
	...
	if locVar.prev == nil {
		...
		self.removeLocVar(locVar.prev) // 递归清理变量
	} else {
		self.locNames[locVar.name] = locVar.prev
	}
}

// addBreakJmp: 把break语句对应的跳转指令添加到最近的循环块中
...
// indexOfUpval：根据变量名获取upval所在的寄存器
