...
func _parseParList(lexer *Lexer) (names []string, isVararg bool) {
	...
	case TOKEN_SEP_RPAREN:
		return nil, false
	case TOKEN_VARARG:
		// TODO:研究下python的编译器在这块是怎么处理的
		// 这里Lua的处理是如果有可变参数直接就不要参数列表了
		lexer.NextToken()
		return nil, true
	}

	names = []string{} // 源码没有初始化 不知道有没有问题 保险起见写一下
...
// parseTableConstructorExp:表构造表达式
...
func _parseFieldList(lexer *Lexer) (ks, vs []Exp) {
	if lexer.LookAhead() != TOKEN_SEP_LCURLY {
